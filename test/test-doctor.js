...
describe.only('doctor', () => {
  ...
  it('throw an error if there is no package file', async () => {
    ...
    return spawn('node', [path.join(__dirname, '../bin/ncu.js'), '--doctor'], { cwd })
  ...
  it('throw an error if --packageData or --packageFile are supplied', async () => {
    ...
    return Promise.all([
      spawn('node', [path.join(__dirname, '../bin/ncu.js'), '--doctor', '--packageFile', 'package.json'])
        .should.eventually.be.rejectedWith('--packageData and --packageFile are not allowed with --doctor'),
      spawn('node', [path.join(__dirname, '../bin/ncu.js'), '--doctor', '--packageData', '{}'])
        .should.eventually.be.rejectedWith('--packageData and --packageFile are not allowed with --doctor')
    ])
  ...
  it('use npm install to restore lockfile if missing', async () => {
    ...
    try
    {
      ...
      const output = await spawn('node', [path.join(__dirname, '../bin/ncu.js'), '--doctor'], { cwd })
